...
func main() {
	...
	s3Client, err := func() (*s3.Client, error) {
		newConfig, err := config.LoadDefaultConfig(context.Background(), config.WithRegion(s3Region))
		if err != nil {
			return nil, err
		}
		return s3.NewFromConfig(newConfig), nil
	}()
	if err != nil {
		log.Fatalf("Failed to initialize s3 client: %v\n", err)
	}
